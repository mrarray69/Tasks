...
namespace IntroducingTasks
{
    class CancelingTasks
    {
        ...
        {
            // it's possible to create a 'composite' cancellation source that involves several tokens
            ...
            // use any of the aforementioned token sources
            emergency.Cancel();
        }
        ...
        private static void CancelableTasks()
        {
            ...
            Task t = new Task(() =>
            {
                ...
                while (true)
                {
                    if (token.IsCancellationRequested) // task cancellation is cooperative, no-one kills your thread
                        break;
                    else
                        Console.WriteLine($"{i++}\t");
                }
